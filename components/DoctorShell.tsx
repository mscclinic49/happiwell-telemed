        ...
        return (
          <Link key={href} href={href} onClick={onClick}
            className={`font-display flex items-center gap-3 px-4 py-3 rounded-[10px] text-sm font-medium transition-colors
              ${active ? 'bg-[var(--hw-green-dk)]/15 text-[var(--hw-green-dk)]' : 'text-[var(--muted)] hover:bg-[var(--hw-green-dk)]/10 hover:text-[var(--foreground)]'}`}>
            <Icon size={19} />
            {label}
          </Link>
        )
    ...
    <div className="flex h-screen overflow-hidden bg-[var(--background)]">
      ...
      <aside className="hidden md:flex flex-col w-56 flex-shrink-0 bg-[var(--card-bg)] border-r border-[var(--border)]">
        <div className="px-5 py-5 border-b border-[var(--border)]">
          <div className="text-xs font-bold uppercase tracking-widest text-[var(--hw-green-dk)] mb-0.5">{'HappiWell'}</div>
        ...
        <div className="px-4 py-4 border-t border-[var(--border)]">
          ...
          <Link href="/doctor/appointments"
            className="flex items-center gap-2 text-xs text-[var(--muted)] hover:text-[var(--hw-green-dk)] transition-colors mb-2">
      ...
      <div className="md:hidden fixed top-0 inset-x-0 z-30 bg-[var(--card-bg)] border-b border-[var(--border)] flex items-center px-4 h-14">
        ...
        <Link href="/doctor/appointments"
          className="relative p-2 rounded-[10px] text-[var(--muted)] hover:text-[var(--hw-green-dk)] hover:bg-[var(--hw-green-dk)]/10 transition-colors mr-1">
          <IconBell size={22} />
          {queueCount > 0 && (
        ...
        <>
          ...
          <aside className="md:hidden fixed left-0 top-0 bottom-0 z-50 w-64 bg-[var(--card-bg)] border-r border-[var(--border)] flex flex-col">
            <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--border)]">
              <div>
                <div className="text-xs font-bold uppercase tracking-widest text-[var(--hw-green-dk)]">{'HappiWell'}</div>
                <div className="text-[11px] text-[var(--muted)]">{'พอร์ทัลแพทย์'}</div>
              </div>
